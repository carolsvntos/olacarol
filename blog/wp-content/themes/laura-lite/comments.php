<?php
	if (!empty($_SERVER['SCRIPT_FILENAME']) && 'comments.php' == basename($_SERVER['SCRIPT_FILENAME']))
		die (esc_html__('Please do not load this page directly. Thanks!','laura'));
	if ( post_password_required() ) { ?>
		<p class="nocomments"><?php esc_html_e('This post is password protected. Enter the password to view comments.','laura'); ?></p>
	<?php
		return;
	}
function laura_comment($comment, $args, $depth) {
   ...
   <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
	...
	<div class = "right-part">
		<div class ="comment-meta">
			<span class="authorBlogName">	
				<?php echo laura_security(get_comment_author_link()); ?>  
			</span>
			<span class = "commentsDate"> <?php printf('%1$s at %2$s', get_comment_date(),  get_comment_time()) ?><?php edit_comment_link(__('(Edit)','laura'),'  ','') ?>
	...
	<?php if ($comment->comment_approved == '0') : ?>
	 <em><?php esc_html_e('Your comment is awaiting moderation.','laura'); ?></em>
	 <br />
	<?php endif; ?>
...
<!-- You can start editing here. -->
	<?php 
		...
		$cancel_reply_link = esc_html__('Cancel Reply','laura');  					
		$title_reply_to = esc_html__('Leave a Reply to','laura'); 			
		$label_submit = esc_html__('Leave a Comment','laura');  		
		$translation_comment_website = esc_html__('Website','laura');  	
		$translation_comment_required = esc_html__('required','laura');  
		$translation_comment_mail = esc_html__('Mail','laura'); 	
		$translation_comment_name = esc_html__('Name','laura');		
		$translation_comment_closed = esc_html__('Comments are closed.','laura');		
	?>
...
<?php if ( have_comments() ) : ?>
	...
	<?php wp_list_comments('type=all&callback=laura_comment'); ?>
	</ol>
...
' ,'laura'), array(  'a' => array( 'href' => array() ) ) ) .'</p>', esc_url(admin_url( 'profile.php' )), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ),
