 ...
 */
add_action( 'widgets_init', 'everly_category_select_slider_post_widget' );
/* Function that registers our widget. */
function everly_category_select_slider_post_widget() {
	register_widget( 'everly_category_select_slider_posts' );
}
class everly_category_select_slider_posts extends WP_Widget {
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'category_select_slider_posts-widget', 'Everly Lite - Premium Posts with slider', $widget_ops, '' );
	}
	function widget( $args, $instance ) {
		...
		<div class="<?php echo $this->id ?>" style="float:left;">
			...
			jQuery( document ).ready(function() {
				var width = jQuery('.<?php echo esc_attr($this->id) ?>').closest('.category_select_slider_posts').width();
				width = width / <?php echo esc_attr($instance['slider']) ?>;
				jQuery('.bxslider-<?php echo esc_attr($this->id); ?>').bxSlider({
					minSlides: <?php echo esc_attr($instance['slider']) ?>,
					maxSlides: <?php echo esc_attr($instance['slider']) ?>,
					slideWidth: width,
			...
			<li>
				<div class="widgett">		
					...
					if ( has_post_thumbnail() ){
						$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()), 'everly_widget', false);
						$image = $image[0];}	
					?>			
					<div class="imgholder">
						<a href="<?php the_permalink() ?>" rel="bookmark" title="Permanent Link to <?php the_title(); ?>">
							<?php if (has_post_thumbnail( get_the_ID() )) echo everly_getImage(get_the_id(),'everly-widget') ;?>		
						</a>
					</div>
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Everly Lite Post slider', 'number' => 3, 'slider' => 2, 'excerpt' => 0, 'n_excerpt' => 10,'category' =>'');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'n_excerpt' )); ?>">Number of words to show in excpert:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'slider' )); ?>">Number of posts to show in slider:</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'category' )); ?>">Category to display</label>
			<?php $args = array(
