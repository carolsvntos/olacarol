  ...
  class OT_Settings {
      ...
      foreach( (array) $this->options as $option ) {
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) ) {
            ...
            /* add styles */
            add_action( 'admin_print_styles-' . $page_hook, array( $this, 'laura_styles' ) );
        ...
        foreach( (array) $this->get_pages( $option ) as $page ) {
          ...
          if ( ! isset( $page['hidden_page'] ) && $screen->id == $this->page_hook[$page['id']] ) {
            ...
            echo '<div class="wrap settings-wrap" id ="page-' . $page['id'] . '">';
			  $theme = wp_get_theme();
              echo '<h2>' . $theme->get( 'Name' ) .' '. $page['page_title'] . '</h2>';
				echo '<div class="pmc-licence">';
				if ( !class_exists( 'EDD_Theme_Updater_Admin' ) ) {
					include(dirname(dirname(dirname( __FILE__ ))) . '/updater/theme-updater-admin.php' );
				}
				...
				$updater = new EDD_Theme_Updater_Admin(
					...
					$config = array(
						...
						'item_name' => 'Laura', // Name of theme
						'theme_slug' => 'laura', // Theme slug
					...
					// Strings
					$strings = array(
						'theme-license' => __( 'Theme License', 'laura' ),
						'enter-key' => __( 'Enter your theme license key.', 'laura' ),
						'license-key' => __( 'License Key', 'laura' ),
						'license-action' => __( 'License Action', 'laura' ),
						'deactivate-license' => __( 'Deactivate License', 'laura' ),
						'activate-license' => __( 'Activate License', 'laura' ),
						'status-unknown' => __( 'License status is unknown.', 'laura' ),
						'renew' => __( 'Renew?', 'laura' ),
						'unlimited' => __( 'unlimited', 'laura' ),
						'license-key-is-active' => __( 'License key is active.', 'laura' ),
						'expires%s' => __( 'Expires %s.', 'laura' ),
						'%1$s/%2$-sites' => __( 'You have %1$s / %2$s sites activated.', 'laura' ),
						'license-key-expired-%s' => __( 'License key expired %s.', 'laura' ),
						'license-key-expired' => __( 'License key has expired. You will not receive updates and you can no longer use support. We recommend you to update the licence so your theme will be up to date in the future.', 'laura' ),
						'license-keys-do-not-match' => __( 'License keys do not match.', 'laura' ),
						'license-is-inactive' => __( 'License is inactive.', 'laura' ),
						'license-key-is-disabled' => __( 'License key is disabled.', 'laura' ),
						'site-is-inactive' => __( 'Site is inactive.', 'laura' ),
						'license-status-unknown' => __( 'License status is unknown.', 'laura' ),
						'update-notice' => __( "Updating this theme will lose any customizations you have made. 'Cancel' to stop, 'OK' to update.", 'laura' ),
						'update-available' => __('<strong>%1$s %2$s</strong> is available. <a href="%3$s" class="thickbox" title="%4s">Check out what\'s new</a> or <a href="%5$s"%6$s>update now</a>.', 'laura' )
					)

				);		
				$updater ->license_page(); 		
				echo '	<h2>Lite version of Laura theme</h2>
		<p>With full version of the theme you get:</p>
		<ul class="freya-lite">
		<li>Revolution slider plugin</li>
		<li>Essential grid plugin</li>
		<li>Ability to change colors via theme options</li>
		<li>Ability to Typography via theme options</li>
		<li>Auto updates</li>
		<li>Premium support</li>		
		</ul>';
				echo '		<a class="buy-freya button" title = "Click to check out the Premium Version" href ="https://premiumcoding.com/wordpress-themes/laura-a-feminine-wordpress-blog-theme/" target = "_blank">Buy Laura now and save 20% with discount code "laura-lite"</a></div>';
              ...
              echo '<div id="option-tree-header-wrap">';
                echo '<ul id="option-tree-header">';
                  ...
                  echo '<li id="pmc-tree-logo"><a href="'.LINK.'"><img src="'.ICON.'"></a></li>';
                  echo '<li id="option-tree-version"><span>' . $theme->get( 'Name' ) . ' '. esc_attr__('Version: ','laura') . $theme->get( 'Version' ).'</span></li>';
              ...
              echo '<div id="option-tree-settings-api">';
                ...
                if ( isset( $page['sections'] ) && count( $page['sections'] ) > 0 ) {
                  ...
                  foreach( (array) $page['sections'] as $section ) {
					$font = '';
					if(!empty($section['font'])) {$font = '<i class="fa '.$section['font'].'"></i>';}
                    echo '<li id="tab_' . $section['id'] . '">'.$font.'<a href="#section_' . $section['id'] . '">' . $section['title'] . '</a></li>';
                  }
    ...
    public function do_settings_fields( $page, $section ) {
      ...
      foreach ( (array) $wp_settings_fields[$page][$section] as $field ) {
        ...
        if ( $field['args']['type'] == 'heading') { $heading = 'heading';} else {$heading = 'ff';} 
        echo '<div id="setting_' . $field['id'] . '" class="'.$heading.' ' . $class . '"' . $conditions . '>';
