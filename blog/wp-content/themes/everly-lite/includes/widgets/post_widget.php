 ...
 */
add_action( 'widgets_init', 'everly_category_select_post_widget' );
/* Function that registers our widget. */
function everly_category_select_post_widget() {
	register_widget( 'everly_category_select_posts' );
}
class everly_category_select_posts extends WP_Widget {
	function __construct() {
		...
		/* Create the widget. */
		parent::__construct( 'category_select_posts-widget', 'Everly Lite - Premium Posts', $widget_ops, '' );
	}
		...
		<div class="widgett <?php if ( has_post_format( 'quote' , get_the_id())) echo 'quote-widget';?> <?php if ( has_post_format( 'link' , get_the_id())) echo 'link-widget';?>">		
			...
			<div class="link-category">
				<div class="blogpostcategory">
					...
					<?php if(everly_getImage(get_the_id(), 'everly_blog') != '') { ?>	
					...
					<div class="blogimage">		
						<a href="<?php echo esc_url($link) ?>" rel="bookmark" title="Permanent Link to <?php the_title_attribute(); ?>"><?php echo everly_getImage(get_the_id(), 'everly_blog'); ?></a>
					</div>
					<?php } ?>												
				</div>
			</div>
			...
			if ( !get_post_format() || has_post_format( 'gallery' , get_the_id())) {
				...
				if ( has_post_thumbnail() ){
					$image = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()),'everly-widget');
					$image = $image[0];}	
					?>			
	...
	function form( $instance ) {
		...
		<p>
		<label for="<?php echo esc_attr($this->get_field_id( 'post' )); ?>">Post to display</label>
		<select name="<?php echo esc_attr($this->get_field_name( 'post' )); ?>" id="<?php echo esc_attr($this->get_field_id( 'post' )); ?>"> <option <?php selected( $instance['post'], 'none'); ?>value="none">Display more then one post</option><?php global $post; $args = array( 'numberposts' => -1); $posts = get_posts($args); foreach( $posts as $post ) : setup_postdata($post); ?><option <?php selected( $instance['post'], $post->ID); ?>value="<?php echo esc_attr($post->ID); ?>"><?php the_title(); ?></option> <?php endforeach; ?> </select> 		
		</p>
		<p>
