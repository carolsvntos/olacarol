...
function optionsframework_admin_init() {
		...
		global $of_options_pmc, $everly_options_machine;
		$everly_options_machine = new Options_Machine($of_options_pmc);
    ...
    if ( isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework' ) {
		if (isset($_REQUEST['of_reset']) && 'reset' == $_REQUEST['of_reset']) {
			...
			if (!wp_verify_nonce($nonce, 'of_ajax_nonce') ) { 
				...
				$defaults = (array) $everly_options_machine->Defaults;
				update_option(EVERLY_OPTIONS,$defaults);
				everly_generate_options_css($defaults); //generate static css file
				header('Location: themes.php?page=optionsframework&reset=true');
				die($everly_options_machine->Defaults);
			} 
		}
    }
}
...
/*-----------------------------------------------------------------------------------*/
/* Options Framework Admin Interface - everly_optionsframework_add_admin */
/*-----------------------------------------------------------------------------------*/
function everly_optionsframework_add_admin() {
	
	if ( !is_plugin_active( 'page-builder-pmc/page-builder-pmc.php' ) ) {
		  //plugin is activated
		$of_page =			add_theme_page('Theme Options','Theme Options','edit_theme_options','optionsframework','everly_optionsframework_options_page') ;
		// Add framework functionaily to the head individually
		add_action("admin_print_scripts-$of_page", 'everly_of_load_only');
		add_action("admin_print_styles-$of_page",'everly_of_style_only');
	} 
		
} 

add_action('admin_menu', 'everly_optionsframework_add_admin');
function everly_everly_add_admin_bar(){
	global $wp_admin_bar;
	$wp_admin_bar->add_menu( array( 'id' => 'everly_theme_options', 'parent' => 'appearance', 'title' => 'PMC Theme options', 'href' => admin_url('admin.php?page=optionsframework') ) );
	
}
add_action('admin_bar_menu', 'everly_everly_add_admin_bar', 1000);
/*-----------------------------------------------------------------------------------*/
/* Build the Options Page - everly_optionsframework_options_page */
/*-----------------------------------------------------------------------------------*/
function everly_optionsframework_options_page(){
global $everly_options_machine;
	/*
	//for debugging
...
<div class="pmc-licence">
		...
		<h2>Lite version of Everly theme</h2>
		<p>With full version of the theme you get:</p>
		<ul class="freya-lite">
		<li>Revolution slider plugin</li>
		<li>Ability to change colors via theme options</li>
		<li>Auto updates</li>
    ...
    <img style="display:none" src="<?php echo EVERLY_ADMIN_DIR; ?>images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
    ...
    <div id="main">
      <div id="of-nav">
        <ul>
          <?php echo $everly_options_machine->Menu ?>
			<li class="import"><a title="Import" href="#import">Import</a></li>
			
			
        </ul>
      </div>
      <div id="content"> <?php echo $everly_options_machine->Inputs /* Settings */ ?> 
		<div class="group" id="import"><h2>Import</h2>
				<div class = "everly-importer-notes">
					<h4>IMPORTANT - READ FIRST</h4>
				</div>
					<ol>
						<li>Never run the importer more then once as it will result in extra (double) content.</li>
						<li>If you have any doubts whether you should run importer or not, do not hesitate to <a target="_blank" href = "https://premiumcoding.com/contact-2/">contact us.</a></li>
					</ol>
...
/*-----------------------------------------------------------------------------------*/
/* Load required styles for Options Page - everly_of_style_only */
/*-----------------------------------------------------------------------------------*/
function everly_of_style_only(){
	wp_enqueue_style('admin-style', EVERLY_ADMIN_DIR . 'admin-style.css');
	wp_enqueue_style('color-picker', EVERLY_ADMIN_DIR . 'css/colorpicker.css');
}	
/*-----------------------------------------------------------------------------------*/
/* Load required javascripts for Options Page - everly_of_load_only */
/*-----------------------------------------------------------------------------------*/
function everly_of_load_only() {
	add_action('admin_head', 'everly_of_admin_head');
	
	wp_enqueue_script('jquery-ui-core');
	wp_enqueue_script('jquery-ui-sortable');
	wp_register_script('jquery-input-mask', EVERLY_ADMIN_DIR .'js/jquery.maskedinput-1.2.2.js', array( 'jquery' ));
	wp_enqueue_script('jquery-input-mask');
	wp_enqueue_script('color-picker-pmc', EVERLY_ADMIN_DIR .'js/colorpicker.js', array('jquery'));
	wp_enqueue_script('ajaxupload', EVERLY_ADMIN_DIR .'js/ajaxupload.js', array('jquery'));

	
		// Registers custom scripts for the Media Library AJAX uploader.
}
function everly_of_admin_head() { 
	...
	<?php  if(isset($_GET["pmc_import"]) && $_GET["pmc_import"] == 'false'){   ?>
		$('.group:last').fadeIn();	
		$('#of-nav li:last').addClass('current');
	<?php } elseif (isset($_GET["pmc_import"]) && $_GET["pmc_import"] == 'finished'){ ?>	
		$('#of-option-themeupdates').fadeIn();	
		$('#of-nav li.themeupdates').addClass('current');	
	<?php } else { ?>	
		$('.group:first').fadeIn();	
	...
	<?php  if(isset($_GET["pmc_import"]) && ($_GET["pmc_import"] == 'true' || $_GET["pmc_import"] == 'false')){   ?>
		//$('#of_save').click();
	<?php if(!$_GET["pmc_import"] == 'false'){ ?>
			window.location.href = "<?php echo admin_url( 'themes.php?page=optionsframework&pmc_import=finished' ) ?>";
		<?php } ?>
	<?php } ?>
	
	$('#import .import').live('click', function(){
		window.location.href = "<?php echo admin_url( 'themes.php?page=optionsframework&pmc_import=start' ) ?>";
	});
...
<?php }
/*-----------------------------------------------------------------------------------*/
/* Ajax Save Action - everly_of_ajax_callback */
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_of_ajax_post_action', 'everly_of_ajax_callback');
function everly_of_ajax_callback() {
	global $everly_options_machine;
	$nonce=$_POST['security'];
	...
	if($save_type == 'upload'){
		...
		$uploaded_file = wp_handle_upload($filename,$override);
			...
			update_option(EVERLY_OPTIONS, $upload_image ) ;
		
				
		 if(!empty($uploaded_file['error'])) {echo 'Upload Error: ' . esc_attr($uploaded_file['error']); }	
	...
	elseif($save_type == 'image_reset'){
			...
			update_option(EVERLY_OPTIONS, $delete_image ) ;
	...
	{
		$everly_data = get_option(BACKUPS);
		update_option(EVERLY_OPTIONS, $everly_data);
		die('1'); 
	}

	elseif ($save_type == 'save') {
		
		parse_str($_POST['data'], $everly_data);
		unset($everly_data['security']);
		unset($everly_data['of_save']);
		parse_str($_POST['data']);
		update_option(EVERLY_OPTIONS, $everly_data);
		everly_generate_options_css($everly_data); //generate static css file
		die('1');
	...
	elseif ($save_type == 'reset') {
		update_option(EVERLY_OPTIONS,$everly_options_machine->Defaults);
        die(1); //options reset
...
/*-----------------------------------------------------------------------------------*/
/* Class that Generates The Options Within the Panel - everly_optionsframework_machine */
/*-----------------------------------------------------------------------------------*/
class Options_Machine {
function __construct($options) {
	
	$return = $this->everly_optionsframework_machine($options);
...
/*-----------------------------------------------------------------------------------*/
/* Generates The Options Within the Panel - everly_optionsframework_machine */
/*-----------------------------------------------------------------------------------*/
public static function everly_optionsframework_machine($options) {
    $everly_data = get_option(EVERLY_OPTIONS);
  ...
  if(isset($_GET["pmc_import"]) && $_GET["pmc_import"] == 'start'){   
		...
		$class_wp_import = get_template_directory() . '/admin/import/plugins/wordpress-importer.php';
		...
		$class_widget_import = get_template_directory() . '/admin/import/plugins/class-widget-data.php';
		...
		$theme_xml = get_template_directory() . '/admin/import/everly.xml';
		...
		if( is_array($menus) ) {
			...
			foreach($menus as $menu) { // assign menus to theme locations
				...
				switch($menu_items->name){
					case 'Main menu':
						$locations['everly_respmenu'] = $menu->term_id;
						$locations['everly_mainmenu'] = $menu->term_id;
						$locations['everly_scrollmenu'] = $menu->term_id;										
						break;										
					case 'Top Menu':
						$locations['everly_topmenu'] = $menu->term_id;									
						break;									
				}						
			}
		}
		...
		/*widgets*/
		$file_widget = get_template_directory() . '/admin/import/widget.json';
	...
	foreach ($options as $value) {
		...
		case 'text':
				if (!isset($everly_data[$value['id']])) {
					$everly_data[$value['id']] = '';
				}
				$textvalue = str_replace('\\','',$everly_data[$value['id']]) ;
		...
		case 'select':
				...
				foreach ($value['options'] as $option) {			
					$output .= '<option value="'.$option.'" ' . selected($everly_data[$value['id']], $option, false) . ' />'.$option.'</option>';	 
				 } 
				 $output .= '</select></div>';
		...
		case 'select2':
				...
				foreach ($value['options'] as $option=>$name) {
					$output .= '<option value="'.$option.'" ' . selected($everly_data[$value['id']], $option, false) . ' />'.$name.'</option>';
				 } 
				$output .= '</select></div>';
		...
		case 'select3':
				...
				foreach( $posts as $post) {
					setup_postdata($post);
					$output .= '<option value="'.get_the_id().'" ' . selected($everly_data[$value['id']], get_the_id(), false) . ' />'.get_the_title().'</option>';
				 } 
				$output .= '</select></div>';
		...
		case 'textarea':	
				...
				if(isset($value['options'])){
					...
					$ta_value = str_replace('\\','',$everly_data[$value['id']]);			
		...
		case "radio":
				if (!isset($everly_data[$value['id']])) {
				 ...
				 foreach($value['options'] as $option=>$name) {
					$output .= '<input class="of-input of-radio" name="'.$value['id'].'" type="radio" value="'.$option.'" ' . checked($everly_data[$value['id']], $option, false) . ' />'.$name.'<br/>';				
				}	 
		...
		case "slidercategory":
				if (!isset($everly_data[$value['id']])) {
					...
					$categories = get_terms("sliderocategory");
						$output .= '<option value="0" ' . selected($everly_data[$value['id']], $category->term_id, false) . ' />None</option>';
					foreach ($categories as $category) {					
						$output .= '<option value="'.$category->term_id.'" ' . selected($everly_data[$value['id']], $category->term_id, false) . ' />'.$category->name.'</option>';
					 } 
					$output .= '</select></div>' ;
				//$output .= wp_dropdown_categories('show_option_all=Show all&hierarchical=2&name='.$value["id"].'&taxonomy=sliderocategory&selected='.selected($everly_data[$value["id"]], $option, false).''); 				
		...
		case 'checkbox':
				...
				$output .= '<input type="checkbox" class="checkbox of-input" name="'.$value['id'].'" id="'. $value['id'] .'" value="1" '. checked($everly_data[$value['id']], true, false) .' />';
		...
		case 'multicheck': 
				...
				$multi_stored = $everly_data[$value['id']];
		...
		case 'upload':
				...
				$output .= Options_Machine::everly_optionsframework_uploader_function($value['id'],$value['std'],$value['mod']);			
		...
		case 'slider':
				...
				$slides = $everly_data[$value['id']];
				$count = count($slides);
				if ($count < 2) {
					$oldorder = 1;
					$order = 1;
					$output .= Options_Machine::everly_optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order,$int,$value['social'],$value['menu'],$value['sidebar']);
				} else {
					$i = 0;
		...
		case 'sorter':
				...
				$sortlists = $everly_data[$value['id']];
		...
		case 'color':	
				...
				$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$everly_data[$value['id']].'"></div></div>';
				$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $everly_data[$value['id']] .'" />';
		...
		case 'colorrgb':		
				...
				$output .= '<div id="' . $value['id'] . '_picker" class="colorSelector"><div style="background-color: '.$everly_data[$value['id']].'" data-color-format="rgb"></div></div>';
				$output .= '<input class="of-color" name="'.$value['id'].'" id="'. $value['id'] .'" type="text" value="'. $everly_data[$value['id']] .'" />';
		...
		case 'sizeColor':
				if (!isset($everly_data[$value['id']])) {
					$everly_data[$value['id']] = array('size' => 12, 'color' => '#000000');
				}
				$sizeColor = $everly_data[$value['id']];
		...
		case 'font':
				if (!isset($everly_data[$value['id']])) {
					$everly_data[$value['id']] = array('face' => 'arial');
				}
				...
				foreach ($faces as $i=>$face) {
					$output .= '<option value="'. $i .'" ' . selected($everly_data[$value['id']], $i, false) . '>'. $face .'</option>';
				}			
		...
		case 'typography':
				if (!isset($everly_data[$value['id']])) {
					$everly_data[$value['id']] = array('size'=>12, 'face' => 'arial', 'color' => '#000000','style'=>'normal');
				}
				$typography_stored = $everly_data[$value['id']];
		...
		case 'border':
				...
				if(!isset($everly_data[$value['id'] . '_width'])) $everly_data[$value['id'] . '_width'] ='';
				if(!isset($everly_data[$value['id'] . '_style'])) $everly_data[$value['id'] . '_style'] ='';
				if(!isset($everly_data[$value['id'] . '_color'])) $everly_data[$value['id'] . '_color'] ='';
				
				$border_stored = array('width' => $everly_data[$value['id'] . '_width'] ,
										'style' => $everly_data[$value['id'] . '_style'],
										'color' => $everly_data[$value['id'] . '_color'],
										);
					
				/* Border Width */
				$border_stored = $everly_data[$value['id']];
		...
		case 'images':
				...
				$select_value = $everly_data[$value['id']];
...
/*-----------------------------------------------------------------------------------*/
/* Aquagraphite Uploader - everly_optionsframework_uploader_function */
/*-----------------------------------------------------------------------------------*/
public static function everly_optionsframework_uploader_function($id,$std,$mod){
    $everly_data =get_option(EVERLY_OPTIONS);
	
	$uploader = '';
	if(!empty($everly_data[$id])) {
    $upload = $everly_data[$id];
	}
	else {
	$upload = '';
	}
	...
	$uploader .= '<div class="upload_button_div"><span class="button image_upload_button" id="'.$id.'">'.esc_html__('Upload','everly-lite').'</span>';
...
/*-----------------------------------------------------------------------------------*/
/* Aquagraphite Slider - everly_optionsframework_slider_function */
/*-----------------------------------------------------------------------------------*/
public static function everly_optionsframework_slider_function($id,$std,$oldorder,$order,$int, $social = false,  $menu = false, $sidebar = false){
    $everly_data = get_option(EVERLY_OPTIONS);
	$slider = '';
 
	
	if(!empty($everly_data[$id])) {
     $slide = $everly_data[$id];
	}
	else {
	 $slide = '';
	}	
	...
	if ($social){
	$slider .= '<label>Icon Font Awesome icon. Only put icon name "fa-facebook-official" </label>';
	}
	else {	
		if(!$menu && !$sidebar){
