 ...
 */
define('ICON','http://laura.premiumcoding.com/wp-content/uploads/2018/01/laura-favicon-1.png');
define('LINK','https://premiumcoding.com');
...
if ( class_exists( 'OT_Loader' ) && defined( 'OT_PLUGIN_MODE' ) && OT_PLUGIN_MODE == true ) {
  ...
  function ot_conflict_notice() {
    ...
    echo '<div class="error"><p>' . __( 'OptionTree is installed as a plugin and also embedded in your current theme. Please deactivate the plugin to load the theme dependent version of OptionTree, and remove this warning.', 'laura' ) . '</p></div>';
  ...
  class OT_Loader {
    ...
    public function load_textdomain() {
      ...
      if ( OT_PLUGIN_MODE ) {
        ...
        load_plugin_textdomain( 'laura', false, OT_LANG_DIR );
        
      } else {
      
        load_theme_textdomain( 'laura', OT_LANG_DIR );
     ...
     * @since     2.0
     */
    private function laura_constants() {
    ...
    private function admin_includes() {
      ...
      /* font Awsome Icons */
      add_action( 'admin_enqueue_scripts', 'ot_load_icons' );	  
     ...
     * @since     2.0
     */
    private function laura_includes() {
    ...
    public function change_image_button( $translation, $text, $domain ) {
      ...
      if ( $pagenow == apply_filters( 'ot_theme_options_parent_slug', 'themes.php' ) && 'default' == $domain && 'Insert into post' == $text ) {
        ...
        return apply_filters( 'ot_upload_text', __( 'Send to OptionTree', 'laura' ) );
