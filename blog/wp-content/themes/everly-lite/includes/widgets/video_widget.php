 ...
 */
add_action( 'widgets_init', 'everly_video_widget' );
/* Function that registers our widget. */
function everly_video_widget() {
	register_widget( 'everly_videos' );
}
class everly_videos extends WP_Widget {
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'videos', 'description' => 'Displays the post image and title.');
		/* Create the widget. */
		parent::__construct( 'videos-widget','Everly Lite - Premium Video', $widget_ops, '' );
		...
		<div class="widgett <?php if(!empty($instance['self_video'])) echo 'self'; ?> id-<?php echo esc_attr($this->id); ?>">	
			<div class="video">
				...
				<?php if(!empty($instance['display_text'])) { ?>
					<div class="wttitle"><a target="_blank" href="<?php echo esc_attr($instance['link']); ?>" rel="bookmark" title="Permanent Link to <?php echo esc_attr($instance['text']); ?>"><?php echo esc_attr($instance['text']);?></a></div>
				<?php } ?>
		</div>	
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => 'Everly Lite Video', 'video' => 'https://www.youtube.com/watch?v=35YZkJaDhh8', 'text' => 'Everly Lite Video Text', 'text-top' => '200px');
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'video' )); ?>">Video URL:</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'self_video' )); ?>">Self hosted video (formats : 'mp4', 'm4v', 'webm', 'ogv', 'wmv', 'flv')?</label>
		</p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_name( 'image' )); ?>">Image cover for self hosted video:</label>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'auto_play' )); ?>">Auto play (only for self hosted video)?</label>
		</p>		
		<p>
		...
		<label for="<?php echo esc_attr($this->get_field_id( 'display_text' )); ?>">Display Text with link?</label>
		</p>		
		<p>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text-top' )); ?>">Text bottom margin:</label>
